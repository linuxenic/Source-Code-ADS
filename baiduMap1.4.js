...
(function() {
	var Twindow = window, Tdocument = document, TMath = Math, TRegExp = RegExp, TparseInt = parseInt, TparseFloat = parseFloat, Tprototype = "prototype", TappendChild = "appendChild", TremoveChild = "removeChild", Tlength = "length", Textend = "extend", Twidth = "width", Theight = "height", ToffsetX = "offsetX", ToffsetY = "offsetY", TaddEventListener = "addEventListener", TremoveEventListener = "removeEventListener", Tstyle = "style", Tanchor = "anchor", TparentNode = "parentNode", Tposition = "position";
	var T, baidu = T = baidu || {
		version : "1.3.4"
	};
	baidu.guid = "$BAIDU$";
	...
	baidu.string.format = function(source, opts) {
		...
		var data = Array[Tprototype].slice.call(arguments, 1), toString = Object[Tprototype].toString;
		if (data[Tlength]) {
			data = data[Tlength] == 1 ? opts !== null
	...
	baidu.dom.removeClass = function(element, className) {
		...
		var oldClasses = element.className.split(/\s+/), newClasses = className
				.split(/\s+/), lenOld, lenDel = newClasses[Tlength], j, i = 0;
		for (; i < lenDel; ++i)
			for (j = 0, lenOld = oldClasses[Tlength]; j < lenOld; ++j)
	...
	baidu.dom.addClass = function(element, className) {
		...
		var classArray = className.split(/\s+/), result = element.className, classMatch = " "
				+ result + " ", i = 0, l = classArray[Tlength];
		for (; i < l; i++)
	...
	baidu.dom.getStyle = function(element, key) {
		...
		if (!value) {
			var fixer = dom._styleFixer[key], style = element.currentStyle
					|| (baidu.browser.ie ? element[Tstyle] : getComputedStyle(
							element, null));
			value = fixer && fixer.get
					? fixer.get(element, style)
					: style[fixer || key]
		}
	...
	baidu.dom.getPosition = function(element) {
		element = baidu.dom.g(element);
		var doc = baidu.dom.getDocument(element), browser = baidu.browser, getStyle = baidu.dom.getStyle, BUGGY_GECKO_BOX_OBJECT = browser.isGecko > 0
				&& doc.getBoxObjectFor
		...
		if (element.getBoundingClientRect) {
			...
			pos.left = TMath.floor(box.left)
					+ TMath.max(doc.documentElement.scrollLeft,
							doc.body.scrollLeft);
			pos.top = TMath.floor(box.top)
					+ TMath.max(doc.documentElement.scrollTop,
							doc.body.scrollTop);
			pos.left -= doc.documentElement.clientLeft;
			pos.top -= doc.documentElement.clientTop;
			var htmlDom = doc.body, htmlBorderLeftWidth = TparseInt(getStyle(
					htmlDom, "borderLeftWidth")), htmlBorderTopWidth = TparseInt(getStyle(
			...
			if (browser.opera > 0 || browser.isWebkit > 0
					&& getStyle(element, "position") == "absolute")
	...
	baidu.lang.Class[Tprototype][TaddEventListener] = function(type, handler,
			key) {
	...
	baidu.dom.hasClass = function(el, className) {
		...
		var res = -1;
		try {
			res = el.className == className
					|| el.className.search(new TRegExp("(\\s|^)" + className
							+ "(\\s|$)"))
		} catch (e) {
			return false
		}
	...
	baidu.enableTapEvent = function() {
		...
		Tap[Tprototype] = {
			...
			move : function(e) {
				stopAndPrevent(e);
				var x = this.hasTouch ? e.touches[0].clientX : e.clientX, y = this.hasTouch
						? e.touches[0].clientY
						: e.clientY;
				if (TMath.abs(x - this.startX) > 10
						|| TMath.abs(y - this.startY) > 10)
					this.moved = true
			},
			end : function(e) {
				stopAndPrevent(e);
	...
	baidu[Textend](Map[Tprototype], {
		...
		_bind : function() {
			...
			me._watchSize = function() {
				...
				if (me[Twidth] != wh[Twidth] || me[Theight] != wh[Theight]) {
					...
					oe.size = originSize;
					me.dispatchEvent(oe);
					me._updateCenterPoint((wh[Twidth] - me[Twidth]) / 2,
							(wh[Theight] - me[Theight]) / 2);
					me.maskLayer.style[Twidth] = (me[Twidth] = wh[Twidth])
							+ "px";
					me.maskLayer.style[Theight] = (me[Theight] = wh[Theight])
							+ "px";
		...
		_updateCenterPoint : function(xn, yn, cp, useCp) {
			...
			var mcp = cp && useCp
					? pro.lngLatToMercator(cp, this.currentCity)
					: this.mercatorCenter;
			if (mcp) {
				this.mercatorCenter = new Point(mcp["lng"] + xn * zu,
						mcp["lat"] - yn * zu);
				var p = pro.mercatorToLngLat(this.mercatorCenter,
						this.currentCity);
				if (p && reAssign)
					this.centerPoint = p
			}
		...
		zoomTo : function(level, point) {
			...
			if (cPoint) {
				...
				this._updateCenterPoint(this[Twidth] / 2 - cPixel.x,
						this[Theight] / 2 - cPixel.y, this.pixelToPoint(cPixel,
								this.lastLevel), true)
			}
		...
		panTo : function(center, opts) {
			if (!(center instanceof Point))
				return;
			this.mercatorCenter = this.projection.lngLatToMercator(center,
					this.currentCity);
			if (Point.isInRange(center))
				this.centerPoint = new Point(center["lng"], center["lat"]);
			else
				this.centerPoint = this.projection.mercatorToLngLat(
		...
		removeTileLayer : function(tilelayer) {
			if (tilelayer)
				this
						.dispatchEvent(new BaseEvent("onremovetilelayer",
								tilelayer))
		},
		...
		centerAndZoom : function(center, zoom) {
			...
			if (isString(center)) {
				...
				loc.setSearchCompleteCallback(function(result) {
							if (loc.getStatus() == 0
									&& loc._json["result"]["type"] == 2) {
								var c = result.getPoi(0)["point"];
								var z = zoom
										|| SUtil.getBestLevel(
												loc._json["content"]["level"],
												me);
			...
			me.mercatorCenter = me.projection.lngLatToMercator(me.centerPoint,
					me.currentCity);
		...
		getSize : function() {
			if (this.customSize && this.customSize instanceof Size)
				return new Size(this.customSize[Twidth],
						this.customSize[Theight]);
			else
		...
		_getProperZoom : function(targetZoom) {
			var levelMin = this.config.minZoom, levelMax = this.config.maxZoom, exceeded = false;
		...
		pointToPixel : function(point, zoom) {
			...
			return this.projection.pointToPixel(point, zoom,
					this.mercatorCenter, this.getSize(), this.currentCity)
		},
		pixelToPoint : function(pixel, zoom) {
			zoom = zoom || this.getZoom();
			return this.projection.pixelToPoint(pixel, zoom,
					this.mercatorCenter, this.getSize(), this.currentCity)
		},
		...
		"pointToPixelFor3D" : function(point, callback) {
			var me = this;
			var mapType = me.mapType, curCity = map.currentCity;
		...
		getBounds : function() {
			if (!this.isLoaded())
				return new Bounds;
			var opts = arguments[0] || {}, margins = opts["margins"]
					|| [0, 0, 0, 0], zoom = opts["zoom"] || null, pt1 = this
					.pixelToPoint({
								x : margins[3],
								y : this[Theight] - margins[2]
							}, zoom), pt2 = this.pixelToPoint({
		...
		_getBestLevel : function(bounds, opts) {
			...
			var margins = opts["margins"] || [10, 10, 10, 10], zoomFactor = opts["zoomFactor"]
					|| 0, ew = margins[1] + margins[3], eh = margins[0]
					+ margins[2], minLevel = mapType.getMinZoom(), maxLevel = mapType
					.getMaxZoom();
			for (var level = maxLevel; level >= minLevel; level--) {
				var zoomUnits = this.getMapType().getZoomUnits(level);
				if (bounds.toSpan()["lng"] / zoomUnits < this[Twidth] - ew
						&& bounds.toSpan()["lat"] / zoomUnits < this[Theight]
								- eh)
					break
			}
			level += zoomFactor;
		...
		getViewport : function(view, opts) {
			...
			if (!view || !view instanceof Bounds && view[Tlength] == 0
					|| view instanceof Bounds && view.isEmpty())
				return curViewport;
			var points = [];
			...
			for (var i = 0, l = points[Tlength]; i < l; i++)
				mercatorPts.push(this.projection.lngLatToMercator(points[i],
						this.currentCity));
			...
			if (opts["margins"]) {
				var margins = opts["margins"], wBias = (margins[1] - margins[3])
						/ 2, hBias = (margins[0] - margins[2]) / 2, zoomUnit = this
						.getMapType().getZoomUnits(zoom);
				if (opts["offset"]) {
		...
		setViewport : function(view, opts) {
			...
			if (viewport.zoom == this.zoomLevel
					&& opts["enableAnimation"] != false) {
				var me = this;
				setTimeout(function() {
		...
		getOverlays : function() {
			var results = [], overlays = this._overlays, customOverlays = this._customOverlays;
			if (overlays)
		...
		_checkZoom : function() {
			var mapTypeMinZoom = this.highResolutionEnabled()
					? this.mapType._opts.highMinZoom
					: this.mapType.getMinZoom();
			var mapTypeMaxZoom = this.highResolutionEnabled()
					? this.mapType._opts.highMaxZoom
					: this.mapType.getMaxZoom();
	...
	var BMAP_API_VERSION = "1.4", BMAP_COORD_LNGLAT = 0, BMAP_COORD_MERCATOR = 1;
	function _addStat(code, opts) {
		...
		var sendStat = function(q) {
			...
			setTimeout(function() {
						_addStat._img.src = MapConfig.imgPath + "blank.gif?"
								+ q.src
					}, 50)
		};
		...
		if (_addStat._sending)
			_addStat._reqQueue.push({
						src : "product=jsapi&v=" + BMap.version + "&t=" + ts
								+ "&code=" + code + extq
					});
		else
			sendStat({
	...
	Animation[Tprototype]._launch = function() {
		...
		me.schedule = me._opts.transition((now - me._beginTime)
				/ me._opts.duration);
		if (isFunction(me._opts.render))
	...
	if (baidu.browser.firefox) {
		baidu[Textend](MapConfig, {
					distCursor : "url(" + MapConfig.imgPath
							+ "ruler.cur),crosshair",
		...
		baidu[Textend](MapConfig, {
					distCursor : "url(" + MapConfig.imgPath
							+ "ruler.cur) 2 6,crosshair",
					defaultCursor : "url(" + MapConfig.imgPath
							+ "openhand.cur) 8 8,default",
					draggingCursor : "url(" + MapConfig.imgPath
	...
	function getScroll() {
		var dd = Tdocument.documentElement, db = Tdocument.body;
	...
	function getDistanceByPixel(px1, px2) {
		if (!px1 || !px2)
			return;
		return TMath.round(TMath.sqrt(TMath.pow(px1.x - px2.x, 2)
				+ TMath.pow(px1.y - px2.y, 2)))
	}
	...
	var b64array = "ABCDEFGHIJKLMNOP" + "QRSTUVWXYZabcdef" + "ghijklmnopqrstuv"
			+ "wxyz0123456789+/" + "=";
	...
	function isSvg() {
		return !!Tdocument.implementation.hasFeature(
	...
	var ModuleVersion = {
		"map" : "20170929031641",
		"tile" : "20170929031641",
		"marker" : "20170929031641",
		"markeranimation" : "20170929031641",
		"poly" : "20170929031641",
		"draw" : "20170929031641",
		"drawbysvg" : "20170929031641",
		"drawbyvml" : "20170929031641",
		"drawbycanvas" : "20170929031641",
		"infowindow" : "20170929031641",
		"oppc" : "20170929031641",
		"opmb" : "20170929031641",
		"menu" : "20170929031641",
		"control" : "20170929031641",
		"navictrl" : "20170929031641",
		"geoctrl" : "20170929031641",
		"copyrightctrl" : "20170929031641",
		"scommon" : "20170929031641",
		"local" : "20170929031641",
		"route" : "20170929031641",
		"othersearch" : "20170929031641",
		"buslinesearch" : "20170929031641",
		"hotspot" : "20170929031641",
		"autocomplete" : "20170929031641",
		"coordtrans" : "20170929031641",
		"coordtransutils" : "20170929031641",
		"mapclick" : "20170929031641"
	};
	baidu.storage = function() {
		var _prefix = "BMap_", ls = window.localStorage, _isSupportLocalStorage = "localStorage" in window
		...
		function _check(moduleName) {
			return _isSupportLocalStorage
					&& !!ls[_getRealyModuleName(moduleName)]
		}
					...
					if (me.delayFlag == false) {
						...
						setTimeout(function() {
							...
							for (var i = 0, l = me.Module._arrMdls[Tlength]; i < l; i++) {
								var mdlName = me.Module._arrMdls[i], mdlCode = "";
							...
							if (mdlsFromServer[Tlength] == 0)
								...
								var modpath = me.Config._baseUrl + "&mod="
										+ mdlsFromServer.join(",");
	...
	Size[Tprototype].equals = function(size) {
		return size && this[Twidth] == size[Twidth]
	...
	function Hotspot(position, opts) {
		...
		this._offsets = opts["offsets"] ? opts["offsets"].slice(0) : [5, 5, 5,
				5];
	...
	baidu[Textend](Control[Tprototype], {
		...
		remove : function() {
			...
			if (!this._container)
				return;
			this._container[TparentNode]
					&& this._container[TparentNode][TremoveChild](this._container);
			this._container._jsobj = null;
			this._container = null
		},
		_render : function() {
			this._container = beforeEndHTML(this._map.container,
		...
		setAnchor : function(anchor) {
			if (this.anchorFixed || !isNumber(anchor) || isNaN(anchor)
	...
	var BMAP_ANCHOR_TOP_LEFT = 0, BMAP_ANCHOR_TOP_RIGHT = 1, BMAP_ANCHOR_BOTTOM_LEFT = 2, BMAP_ANCHOR_BOTTOM_RIGHT = 3;
	var BMAP_NAVIGATION_CONTROL_LARGE = 0, BMAP_NAVIGATION_CONTROL_SMALL = 1, BMAP_NAVIGATION_CONTROL_PAN = 2, BMAP_NAVIGATION_CONTROL_ZOOM = 3;
	...
	baidu[Textend](NavigationControl[Tprototype], {
				...
				setType : function(type) {
					if (isNumber(type) && type >= BMAP_NAVIGATION_CONTROL_LARGE
	...
	baidu.object[Textend](CopyrightControl[Tprototype], {
				...
				addCopyright : function(copyright) {
					if (!copyright || !isNumber(copyright["id"])
							|| isNaN(copyright["id"]))
						return;
					var ncpy = {
						"bounds" : null,
	...
	var BMAP_UNIT_METRIC = "metric", BMAP_UNIT_IMPERIAL = "us";
	...
	baidu.object[Textend](ScaleControl[Tprototype], {
				...
				setUnit : function(unit) {
					this._opts.unit = this._units[unit]
							&& this._units[unit].name || this._opts.unit
				},
				getUnit : function() {
	...
	var BMAP_MAPTYPE_CONTROL_HORIZONTAL = 0, BMAP_MAPTYPE_CONTROL_DROPDOWN = 1;
	...
	baidu.object[Textend](ContextMenu[Tprototype], {
				...
				addItem : function(item) {
					if (!item || item._type != "menuitem" || item._text == ""
							|| item._width <= 0)
						return;
				...
				removeSeparator : function(index) {
					if (!this._dividers[index])
						return;
					for (var i = 0, l = this._items[Tlength]; i < l; i++) {
						if (this._items[i] && this._items[i]._type == "divider"
								&& this._items[i]._dIndex == index) {
							this._items.splice(i, 1);
							l--
						}
						if (this._items[i] && this._items[i]._type == "divider"
	...
	baidu.object[Textend](Bounds[Tprototype], {
				...
				containsBounds : function(bounds) {
					if (!(bounds instanceof Bounds) || this.isEmpty()
							|| bounds.isEmpty())
						return false;
					return bounds._swLng > this._swLng
							&& bounds._neLng < this._neLng
							&& bounds._swLat > this._swLat
							&& bounds._neLat < this._neLat
				},
				getCenter : function() {
					if (this.isEmpty())
						return null;
					return new Point((this._swLng + this._neLng) / 2,
							(this._swLat + this._neLat) / 2)
				},
				intersects : function(bounds) {
					if (!(bounds instanceof Bounds))
						return null;
					if (TMath.max(bounds._swLng, bounds._neLng) < TMath.min(
							this._swLng, this._neLng)
							|| TMath.min(bounds._swLng, bounds._neLng) > TMath
									.max(this._swLng, this._neLng)
							|| TMath.max(bounds._swLat, bounds._neLat) < TMath
									.min(this._swLat, this._neLat)
							|| TMath.min(bounds._swLat, bounds._neLat) > TMath
									.max(this._swLat, this._neLat))
						return null;
					...
					return new Bounds(new Point(newMinX, newMinY), new Point(
									newMaxX, newMaxY))
				},
				containsPoint : function(point) {
					if (!(point instanceof Point) || this.isEmpty())
						return false;
					return point["lng"] >= this._swLng
							&& point["lng"] <= this._neLng
							&& point["lat"] >= this._swLat
							&& point["lat"] <= this._neLat
				},
				...
				toSpan : function() {
					...
					return new Point(TMath.abs(this._neLng - this._swLng),
							TMath.abs(this._neLat - this._swLat))
				}
			});
	...
	Point.isInRange = function(pt) {
		return pt && pt["lng"] <= 180 && pt["lng"] >= -180 && pt["lat"] <= 74
				&& pt["lat"] >= -74
	};
	Point[Tprototype].equals = function(other) {
		return other && this["lat"] == other["lat"]
				&& this["lng"] == other["lng"]
	};
	function Projection() {
	}
	...
	baidu[Textend](MercatorProjection, {
		...
		LL2MC : [
				...
				[8.277824516172526E-4, 111320.7020463578, 6.477955746671607E8,
						-4.082003173641316E9, 1.077490566351142E10,
						-1.517187553151559E10, 1.205306533862167E10,
						-5.124939663577472E9, 9.133119359512032E8, 67.5],
				[0.00337398766765, 111320.7020202162, 4481351.045890365,
						-2.339375119931662E7, 7.968221547186455E7,
						-1.159649932797253E8, 9.723671115602145E7,
						-4.366194633752821E7, 8477230.501135234, 52.5],
				[0.00220636496208, 111320.7020209128, 51751.86112841131,
						3796837.749470245, 992013.7397791013,
						-1221952.21711287, 1340652.697009075,
						-620943.6990984312, 144416.9293806241, 37.5],
				[-3.441963504368392E-4, 111320.7020576856, 278.2353980772752,
						2485758.690035394, 6070.750963243378,
						54821.18345352118, 9540.606633304236,
		...
		convertMC2LL : function(point) {
			...
			var point = new Point(lnglat["lng"].toFixed(6), lnglat["lat"]
							.toFixed(6));
			return point
		},
		...
		convertor : function(fromPoint, factor) {
			...
			var y = factor[2] + factor[3] * temp + factor[4] * temp * temp
					+ factor[5] * temp * temp * temp + factor[6] * temp * temp
					* temp * temp + factor[7] * temp * temp * temp * temp
					* temp + factor[8] * temp * temp * temp * temp * temp
					* temp;
		...
		getDistance : function(x1, x2, y1, y2) {
			return this.EARTHRADIUS
					* TMath.acos(TMath.sin(y1) * TMath.sin(y2) + TMath.cos(y1)
							* TMath.cos(y2) * TMath.cos(x2 - x1))
		},
	...
	baidu[Textend](MercatorProjection[Tprototype], {
				...
				pointToPixel : function(point, zoom, mapCenter, mapSize,
						curCity) {
					if (!point)
						return;
					point = this.lngLatToMercator(point, curCity);
					var zoomUnits = this.getZoomUnits(zoom);
					var x = TMath.round((point["lng"] - mapCenter["lng"])
							/ zoomUnits + mapSize[Twidth] / 2);
					var y = TMath.round((mapCenter["lat"] - point["lat"])
							/ zoomUnits + mapSize[Theight] / 2);
					return new Pixel(x, y)
				},
				pixelToPoint : function(pixel, zoom, mapCenter, mapSize,
						curCity) {
					if (!pixel)
						return;
					var zoomUnits = this.getZoomUnits(zoom);
					var lng = mapCenter["lng"] + zoomUnits
							* (pixel.x - mapSize[Twidth] / 2);
					var lat = mapCenter["lat"] - zoomUnits
							* (pixel.y - mapSize[Theight] / 2);
	...
	baidu[Textend](PerspectiveProjection[Tprototype], {
		lngLatToMercator : function(lngLat, mapCity) {
			return this._convert2DTo3D(mapCity, MercatorProjection
							.convertLL2MC(lngLat))
		},
		mercatorToLngLat : function(mercator, mapCity) {
			return MercatorProjection.convertMC2LL(this._convert3DTo2D(mapCity,
					mercator))
		},
		"lngLatToPointFor3D" : function(lngLat, callback) {
			...
			ModuleMgr.load("coordtrans", function() {
						var obj = CoordTrans.getOMap_pts(
								me.curCityName || "bj", mcPt);
		...
		"pointToLngLatFor3D" : function(point, callback) {
			...
			ModuleMgr.load("coordtrans", function() {
						var obj = CoordTrans.getMapJw_pts(me.curCityName
										|| "bj", pixPt);
	...
	BMap.register(function(map) {
				var t = map.temp;
				t.overlayDiv = map.overlayDiv = _createOverlayDiv(map.platform,
						200);
				map._panes.floatPane = _createOverlayDiv(t.overlayDiv, 800);
				map._panes.markerMouseTarget = _createOverlayDiv(t.overlayDiv,
						700);
				...
				function _createOverlayDiv(container, zIndex) {
					var div = create("div"), s = div.style;
					s[Tposition] = "absolute";
	...
	BMap.register(function(map) {
		...
		if (baidu.browser.ie && baidu.browser.ie < 8
				|| Tdocument.compatMode == "BackCompat")
	...
	Graph.getGraphPoints = function(points) {
		...
		if (Object[Tprototype].toString.apply(points) == "[object Array]"
				&& points[Tlength] > 0)
			result = points;
		return result
	};
	...
	baidu[Textend](Graph[Tprototype], {
				...
				remove : function() {
					if (this.map)
						this.map[TremoveEventListener]("onmousemove",
	...
	baidu[Textend](Division[Tprototype], {
		_addDom : function() {
			...
			if (config.borderColor != "none")
				html.push("border:" + config.lineStroke + "px "
						+ config.lineStyle + " " + config.lineColor + ";");
			html.push("opacity:" + config.opacity + "; filter:(opacity="
					+ config.opacity * 100 + ")");
			html.push("background:" + config.background + ";");
			html.push('z-index:60;">');
			html.push(content);
			html.push("</div>");
			this.domElement = beforeEndHTML(
					this.map.getPanes().markerMouseTarget, html.join(""))
		},
		"initialize" : function(map) {
			this.map = map;
			this._addDom();
		...
		"draw" : function() {
			...
			this._config.offset = new Size(-TMath.round(this._config[Twidth]
							/ 2)
							- TMath.round(this._config.lineStroke), -TMath
							.round(this._config[Theight] / 2)
							- TMath.round(this._config.lineStroke));
			this.domElement.style.left = pix.x + this._config.offset[Twidth]
					+ "px";
			this.domElement.style.top = pix.y + this._config.offset[Theight]
					+ "px"
		},
		getPosition : function() {
	...
	function Icon(imageUrl, size, opts) {
		...
		this["size"] = size;
		var defaultAnchor = new Size(TMath.floor(size[Twidth] / 2), TMath
						.floor(size[Theight] / 2));
		opts = opts || {};
		var _config = {
	...
	baidu[Textend](Icon[Tprototype], {
				...
				setInfoWindowAnchor : function(size) {
					if (!size)
						return;
					this["infoWindowAnchor"] = new Size(size[Twidth],
							size[Theight])
				},
	...
	function InfoWindow(content, opts) {
		...
		this._config = {
			...
			enableCloseOnClick : opts["enableCloseOnClick"] === false
					? false
					: true,
			margin : [10, 10, 40, 10],
	...
	baidu[Textend](Label[Tprototype], {
				...
				setPosition : function(point) {
					if (point instanceof Point && !this.getMarker())
						this.point = this._config[Tposition] = new Point(
								point["lng"], point["lat"])
				},
				setContent : function(content) {
					this.content = content
				...
				setStyle : function(styles) {
					styles = styles || {};
					this._config.styles = baidu[Textend](this._config.styles,
							styles)
				},
				...
				setMarker : function(marker) {
					this._marker = marker;
					if (marker)
						this.point = this._config[Tposition] = marker
								.getPosition();
					else
	...
	var BMAP_ANIMATION_DROP = 1, BMAP_ANIMATION_BOUNCE = 2;
	var DEFAULT_ICON = new Icon(MapConfig.imgPath + "marker_red_sprite.png",
			new Size(19, 25), {
				"anchor" : new Size(10, 25),
				"infoWindowAnchor" : new Size(10, 0)
			});
	var DEFAULT_SHADOW = new Icon(MapConfig.imgPath + "marker_red_sprite.png",
			new Size(20, 11), {
	...
	function Marker(point, opts) {
		...
		this._config = {
			...
			draggingCursor : opts["draggingCursor "]
					|| MapConfig.draggingCursor
		};
	...
	function Polygon(points, opts) {
		...
		opts = opts || {};
		this._config.fillOpacity = opts["fillOpacity"]
				? opts["fillOpacity"]
				: 0.65;
		if (opts.fillColor == "")
			this._config.fillColor = "";
		else
			this._config.fillColor = opts["fillColor"]
					? opts["fillColor"]
					: "#fff";
		this.setPath(points);
		var me = this;
	...
	baidu[Textend](Polygon[Tprototype], {
				setPath : function(points, flag) {
					...
					if (pts[Tlength] > 1
							&& !pts[0].equals(pts[pts[Tlength] - 1]))
				...
				setPositionAt : function(index, point) {
					if (!this._userPoints[index])
						return;
					this._userPoints[index] = new Point(point["lng"],
							point["lat"]);
					this.points[index] = new Point(point["lng"], point["lat"]);
					if (index == 0
							&& !this.points[0]
									.equals(this.points[this.points[Tlength]
											- 1]))
						this.points[this.points[Tlength] - 1] = new Point(
								point["lng"], point["lat"]);
					this._calcBounds()
				},
				getPath : function() {
	...
	baidu[Textend](Circle[Tprototype], {
		...
		_getPerimeterPoints : function(center, radius) {
			if (!center || !radius || !this.map)
				return [];
			var map = this.map;
			var cx = center["lng"], cy = center["lat"];
			var points = [];
			var d = radius / 6378800, lat1 = TMath.PI / 180 * cy, lng1 = TMath.PI
					/ 180 * cx;
			for (var i = 0; i < 360; i += 9) {
				var tc = TMath.PI / 180 * i, y = TMath.asin(TMath.sin(lat1)
						* TMath.cos(d) + TMath.cos(lat1) * TMath.sin(d)
						* TMath.cos(tc)), dlng = TMath.atan2(TMath.sin(tc)
								* TMath.sin(d) * TMath.cos(lat1), TMath.cos(d)
								- TMath.sin(lat1) * TMath.sin(y)), x = (lng1
						- dlng + TMath.PI)
						% (2 * TMath.PI) - TMath.PI, point = new Point(x
								* (180 / TMath.PI), y * (180 / TMath.PI));
				points.push(point)
			}
	...
	baidu[Textend](TileMgr[Tprototype], {
		...
		showTile : function(tileInfo, centerPos, tilelayer) {
			...
			if (tile && tile.img) {
				tilelayer.tilesDiv.insertBefore(tile.img,
						tilelayer.tilesDiv.lastChild);
			...
			var zoomUnits = 256
					* TMath.pow(2, mapType.getMaxZoom() - tileInfo[2]);
			var point = new Point(tileInfo[0] * zoomUnits, tileInfo[1]
							* zoomUnits);
				...
				this._checkLoadedTimer = setTimeout(function() {
					if (me.numLoading == 0) {
						...
						if (isFirstTilesLoaded) {
							if (TIME_LINE.startLoadScript
									&& TIME_LINE.loadScriptEnd
									&& TIME_LINE.loadedMap) {
								var currentTime = TIME_LINE.firstTilesLoaded = getCurrentTime();
								var size = me.map.getSize();
								setTimeout(function() {
									_addStat(5030, {
										"load_script_time" : TIME_LINE.loadScriptEnd
												- TIME_LINE.startLoadScript,
										"load_tiles_time" : currentTime
												- TIME_LINE.loadedMap,
										"map_width" : size[Twidth],
										"map_height" : size[Theight],
										"map_size" : size[Twidth]
												* size[Theight]
									})
								}, 1E4)
							}
		...
		getTileName : function(tileInfo, tilelayer) {
			if (this.map.getMapType() === BMAP_PERSPECTIVE_MAP)
				return "TILE-" + tilelayer.guid + "-" + this.map.cityCode + "-"
						+ tileInfo[0] + "-" + tileInfo[1] + "-" + tileInfo[2];
			else
				return "TILE-" + tilelayer.guid + "-" + tileInfo[0] + "-"
						+ tileInfo[1] + "-" + tileInfo[2]
		},
		...
		moveGridTiles : function() {
			var me = this, map = me.map, mapType = map.getMapType();
			if (mapType == BMAP_PERSPECTIVE_MAP)
				ModuleMgr.load("coordtrans", function() {
							me._moveGridTiles()
						}, true);
			...
			for (var n = 0; n < layernums; n++) {
				...
				if (mapType == BMAP_PERSPECTIVE_MAP)
					if (centerPoint.equals(new Point(0, 0)))
						centerPoint = map.mercatorCenter = projection
								.lngLatToMercator(map.centerPoint,
										map.currentCity);
				...
				var cell = [
						row,
						column,
						(centerPoint["lng"] - row * levelUnits) / levelUnits
								* tileSize,
						(centerPoint["lat"] - column * levelUnits) / levelUnits
								* tileSize];
				var fromRow = cell[0]
						- TMath.ceil((map[Twidth] / 2 - cell[2]) / tileSize);
				var fromColumn = cell[1]
						- TMath.ceil((map[Theight] / 2 - cell[3]) / tileSize);
				var toRow = cell[0]
						+ TMath.ceil((map[Twidth] / 2 + cell[2]) / tileSize);
				var addC = 0;
				...
				for (var tileName in mapTiles) {
					...
					if (info[2] != zoom
							|| info[2] == zoom
							&& (fromRow > info[0] || toRow <= info[0]
									|| fromColumn > info[1] || toColumn <= info[1]))
						this.hideTile(tile)
				}
				var divLeft = -map[ToffsetX] + map[Twidth] / 2;
				var divTop = -map[ToffsetY] + map[Theight] / 2;
				tilelayer.tilesDiv.style.left = TMath
						.ceil(centerPixX + divLeft)
						- centerPosition[0] + "px";
				tilelayer.tilesDiv.style.top = TMath.ceil(centerPixY + divTop)
						- centerPosition[1] + "px";
				var tilesOrder = [];
				...
				tilesOrder.sort(function(center) {
							return function(x, y) {
								return 0.4
										* TMath.abs(x[0] - center[0])
										+ 0.6
										* TMath.abs(x[1] - center[1])
										- (0.4 * TMath.abs(y[0] - center[0]) + 0.6
												* TMath.abs(y[1] - center[1]))
							}
						}([cell[0] - 1, cell[1] - 1]));
				this.numLoading += tilesOrder[Tlength];
				for (var i = 0, l = tilesOrder[Tlength]; i < l; i++)
					this.showTile([tilesOrder[i][0], tilesOrder[i][1], zoom],
							centerPosition, tilelayer)
			}
			return
		},
			...
			if (me.img && !isInDocument(me.img))
				if (tilelayer.tilesDiv) {
					...
					if (baidu.browser.ie <= 6 && baidu.browser.ie > 0
							&& me._transparentPng)
						me.img.style.cssText += ';filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(src="'
								+ me.src + '",sizingMethod=scale);'
				}
	...
	Tile[Tprototype]._load = function() {
		if (baidu.browser.ie > 0 && baidu.browser.ie <= 6
				&& this._transparentPng)
	...
	baidu[Textend](TileLayer[Tprototype], {
				initialize : function(map, container) {
					...
					if (!this.tilesDiv) {
						...
						style.left = TMath.ceil(-map[ToffsetX] + map[Twidth]
								/ 2)
								+ "px";
						style.top = TMath.ceil(-map[ToffsetY] + map[Theight]
								/ 2)
								+ "px";
						container[TappendChild](div);
						this.tilesDiv = div
				...
				"getTilesUrl" : function(tileCoord, zoom) {
					...
					if (this.opts["tileUrlTemplate"]) {
						url = this.opts["tileUrlTemplate"].replace(/\{X\}/,
								tileCoord.x);
	...
	TrafficLayer[Tprototype]["getTilesUrl"] = function(point, level) {
		var pDate = "";
		if (this._opts["predictDate"])
			pDate = "HistoryService?" + "day="
					+ (this._opts["predictDate"]["weekday"] - 1) + "&hour="
					+ this._opts["predictDate"]["hour"] + "&t="
					+ (new Date).getTime() + "&";
		else {
		...
		var map = this._map, row = point.x, column = point.y, px = TMath
				.floor(row / 200), py = TMath.floor(column / 200), url = this._tileUrl
				+ pDate + "level=" + level + "&x=" + row + "&y=" + column;
	...
	var TILE_BASE_URLS = ["http://shangetu0.map.bdimg.com/it/",
	...
	var TILE_ONLINE_URLS = ["http://online0.map.bdimg.com/tile/",
	...
	normalMapTileLayer["getTilesUrl"] = function(tileCoord, zoom) {
		...
		if (this.map.highResolutionEnabled())
			styles = "ph";
		var url = TILE_ONLINE_URLS[TMath.abs(row + column)
				% TILE_ONLINE_URLS[Tlength]]
				+ "?qt=tile&x="
				+ (row + "").replace(/-/gi, "M")
				+ "&y="
				+ (column + "").replace(/-/gi, "M")
				...
				+ "&styles="
				+ styles
				+ (baidu.browser.ie == 6 ? "&color_dep=32&colors=50" : "")
				+ "&udt=" + udt;
		return url.replace(/-(\d+)/gi, "M$1")
	};
	...
	PerspectiveTileLayer["getTilesUrl"] = function(tileCoord, zoom) {
		...
		url = this.tileUrls[TMath.abs(row + column) % this.tileUrls[Tlength]]
				+ this.map.currentCity + "/" + this.map.cityCode + "/3/" + "lv"
				+ (21 - zoom) + "/" + row + "," + column + "." + "jpg";
		return url
	};
	var BMAP_PERSPECTIVE_MAP = new MapType("\u4e09\u7ef4",
			PerspectiveTileLayer, {
				"tips" : "\u663e\u793a\u4e09\u7ef4\u5730\u56fe",
				"minZoom" : 15,
				"maxZoom" : 20,
				"textColor" : "white",
	...
	satelliteTileLayer["getTilesUrl"] = function(tileCoord, zoom) {
		var row = tileCoord.x;
		var column = tileCoord.y;
		var url = TILE_BASE_URLS[TMath.abs(row + column)
				% TILE_BASE_URLS[Tlength]]
				+ "u=x="
				+ row
				+ ";y="
				+ column
				+ ";z="
				+ zoom
				+ ";v=009;type=sate&fm=46&udt=20141015";
		return url.replace(/-(\d+)/gi, "M$1")
	};
	...
	var BMAP_HYBRID_MAP = new MapType("\u6df7\u5408", [satelliteTileLayer,
					streetLayer], {
				"tips" : "\u663e\u793a\u5e26\u6709\u8857\u9053\u7684\u536b\u661f\u5f71\u50cf",
				"labelText" : "\u8def\u7f51",
				"minZoom" : 1,
	...
	var BMAP_POI_TYPE_NORMAL = 0, BMAP_POI_TYPE_BUSSTOP = 1, BMAP_POI_TYPE_BUSLINE = 2, BMAP_POI_TYPE_SUBSTOP = 3, BMAP_POI_TYPE_SUBLINE = 4;
	...
	function BaseSearch(loc, opts) {
		...
		if (typeof opts != "undefined"
				&& typeof opts["renderOptions"] != "undefined"
				&& typeof opts["renderOptions"]["autoViewport"] != "undefined")
			this._opts.renderOptions.autoViewport = opts["renderOptions"]["autoViewport"];
		else
			this._opts.renderOptions.autoViewport = true;
		this._opts.renderOptions.panel = baidu
				.G(this._opts.renderOptions.panel)
	}
	...
	var SearchRequestMgr = {
		...
		request : function(cbk, params, userData, path, disableLog) {
			...
			var me = this, url = me.REQ_BASE_URL + path + "?" + reqParam
	...
	var BMAP_STATUS_SUCCESS = 0, BMAP_STATUS_CITY_LIST = 1, BMAP_STATUS_UNKNOWN_LOCATION = 2, BMAP_STATUS_UNKNOWN_ROUTE = 3, BMAP_STATUS_INVALID_KEY = 4, BMAP_STATUS_INVALID_REQUEST = 5, BMAP_STATUS_PERMISSION_DENIED = 6, BMAP_STATUS_SERVICE_UNAVAILABLE = 7, BMAP_STATUS_TIMEOUT = 8;
	var BMAP_ROUTE_TYPE_WALKING = 2, BMAP_ROUTE_TYPE_DRIVING = 3;
	var BMAP_ROUTE_STATUS_NORMAL = 0, BMAP_ROUTE_STATUS_EMPTY = 1, BMAP_ROUTE_STATUS_ADDRESS = 2;
	...
	var LocalSearch = function(loc, opts) {
		...
		if (typeof opts["renderOptions"]["selectFirstResult"] != "undefined"
				&& !opts["renderOptions"]["selectFirstResult"])
	...
	baidu[Textend](LocalSearch[Tprototype], {
				...
				setPageCapacity : function(cap) {
					if (typeof cap == "number" && !isNaN(cap))
						this._opts.pageCapacity = cap < 1
								? LocalSearch.DEFAULT_PAGE_CAPACITY
								: cap > LocalSearch.MAX_PAGE_CAPACITY
										? LocalSearch.DEFAULT_PAGE_CAPACITY
										: cap;
					else
	...
	var BMAP_TRANSIT_POLICY_LEAST_TIME = 0, BMAP_TRANSIT_POLICY_LEAST_TRANSFER = 2, BMAP_TRANSIT_POLICY_LEAST_WALKING = 3, BMAP_TRANSIT_POLICY_AVOID_SUBWAYS = 4;
	var BMAP_LINE_TYPE_BUS = 0, BMAP_LINE_TYPE_SUBWAY = 1, BMAP_LINE_TYPE_FERRY = 2;
	...
	TransitRoute.LINE_TYPE_MAPPING = [0, 1, 0, 0, 0, 0, 0, 0, 2, 0, 0, 0, 1, 1,
			1];
	...
	var BMAP_HIGHLIGHT_STEP = 1, BMAP_HIGHLIGHT_ROUTE = 2;
	var DWRoute = function(location, opts) {
		...
		if (renderOptions.highlightMode != BMAP_HIGHLIGHT_STEP
				&& renderOptions.highlightMode != BMAP_HIGHLIGHT_ROUTE)
	...
	var BMAP_DRIVING_POLICY_LEAST_TIME = 0, BMAP_DRIVING_POLICY_LEAST_DISTANCE = 1, BMAP_DRIVING_POLICY_AVOID_HIGHWAYS = 2;
