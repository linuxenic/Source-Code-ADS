...
<html>
  <head>
	  ...
	  <link href="page/auth/qrlogin/qrlogin.css" rel="stylesheet" type="text/css" />
	  ...
	  <style type="text/css">
	   ...
	   <%
	    ...
	    if("0".equals(code)){
	   %>
		  <script type="text/javascript">var BASEPATH = '<%=basePath%>';var AUTH_TBAR  = '<%=request.getAttribute("auth")%>';var msgdata = '<%=msg%>';</script>
		  <script type="text/javascript" src="page/auth/qrlogin/qrlogin.js"></script>
	   <%
	    }
	   %>	  
  ...
  <body>
   ...
   if("2".equals(code)){
    %>
                 <h3>&nbsp;您还未绑定过此服务器的任何账号。<font color="gray" size="2px"> <hr/>1、绑定流程： 请使用账号密码登陆系统，进入“用户设置”-》用户信息中心，然后点击“绑定二维码”按钮，使用微信扫一扫执行绑定;<br/>
  2、扫码登陆： 在登陆界面右下角点击二维码小图标，将会显示登陆二维码，使用微信扫一扫可登陆;<br/>
  3、解除绑定： 请使用账号密码登陆系统，进入“用户设置”-》用户信息中心，然后点击“点击显示二维码”按钮，使用微信扫一扫执行解除绑定<br/>
  </font></h3>
  <font color="red" size="2">【此功能需要服务器有扫码登陆的授权 ，并且将服务器接入互联网！】</font>
