Ext.onReady(function () {
    ...
    var cutActivityViewport, cutActivityGridPanel, cutActivityDataStore, cutActivityCtrColumn, cutActivityAjaxProxy,
        cutActivityWin, rankingWin, rankingGridPanel, rankingDataStore, rankingAjaxProxy, rankingSelModel;
    var userIds = new Ext.util.MixedCollection(),is2Coupon = false;
    var dateFormat='Y/m/d H:i:s';
    cutActivityCtrColumn = {
    ...
    // if (AUTH["update"]) {
    //     cutActivityCtrColumn.items.push({
    //         iconCls: 'editIconCss'
    //         , tooltip: window.top.getIi18NText('modify')
    //         ,handler:  editcutActivity
    //     });
    //     cutActivityCtrColumn.hidden = false;
    // }
    ...
    if (AUTH["delete"]) {
        cutActivityCtrColumn.items.push({
            ...
            ,handler:  delcutActivity
        });
        cutActivityCtrColumn.hidden = false;
    }
    function delcutActivity(v, r, c, i, e, record, row){
        ...
        if(status === 1){
            ...
            Ext.Msg.show({
                ...
                fn: function (bid, text, opt) {
                    if (bid === 'ok') {
                        Ext.Ajax.request({
                            url: 'shared!delcutActivityRecord.action'
                            , method: 'post'
                            ,params: {ids: ids}
    ...
    cutActivityViewport = Ext.create("Ext.container.Viewport", {
        layout: 'border',
        ...
        items: [{
            ...
            , items: [
                {
                    ...
                    , labelWidth: 40
                    , margin: '8 2 5 43'
                    , emptyText: getIi18NText('activityName')+','+getIi18NText('activityShareCode')+','+getIi18NText('terGoodsName')
                    +','+getIi18NText('wechatName')
                }, {
                    xtype: 'button'
            ...
            , layout: 'fit'
            , listeners: {
                render: rendercutActivityTable
            }
        }]
    });

    function rendercutActivityTable($this, eopt) {
        cutActivityAjaxProxy = getAjaxProxy({url: 'shared!getAllCutActivityRecord.action'});
        cutActivityDataStore = Ext.create('Ext.data.Store', {
            fields: ["carId","actName","actStatus","gcode","gname","price","openid","nickName","amount","shareCode",
                     "sponsors","status","createTime","total"],
            buffered: false,
            autoLoad: true,
            pageSize: 10,
            leadingBufferZone: 50,
            proxy: cutActivityAjaxProxy
            , listeners: {}
        });
        cutActivityGridPanel = Ext.create("Ext.grid.Panel", {
            title: window.top.getIi18NText('cutActivityRecord'),
            iconCls: 'tabIconCss',
            store: cutActivityDataStore,
            frame: false,
            border: false,
            forceFit: true,
            autoScroll: true,
            columns: {
                items: [
                    ...
                    {text: getIi18NText('activityName'), dataIndex: 'actName', minWidth: 150, flex: 1, align: 'center'},
//                    {text: getIi18NText('activityStatus'), dataIndex: 'actStatus', minWidth: 150, flex: 1, align: 'center',renderer:statusFmtRender},
                    {text: 'openId', dataIndex: 'openid', minWidth: 150, flex: 1, align: 'center'},
                    {text: getIi18NText('wechatName'), dataIndex: 'nickName', minWidth: 150, flex: 1, align: 'center'},
                    {text: getIi18NText('goodsPrice'), dataIndex: 'price', minWidth: 150, flex: 1, align: 'center'},
                    {text: getIi18NText('cutTotal'), dataIndex: 'total', minWidth: 150, flex: 1, align: 'center'},
                    {text: getIi18NText('cutStatus'), dataIndex: 'status', minWidth: 150, flex: 1, align: 'center',renderer:activityFmtRender},
                    {text: getIi18NText('activitySponsors'), dataIndex: 'sponsors', minWidth: 150, flex: 1, align: 'center',renderer:sponsorsFmtRender},
                    {text: getIi18NText('goodsCode'), dataIndex: 'gcode', minWidth: 150, flex: 1, align: 'center'},
                    {text: getIi18NText('terGoodsName'), dataIndex: 'gname', minWidth: 150, flex: 1, align: 'center'},
                    {text: getIi18NText('cutAmount'), dataIndex: 'amount', minWidth: 150, flex: 1, align: 'center'},
                    {text: getIi18NText('activityShareCode'), dataIndex: 'shareCode', minWidth: 150, flex: 1, align: 'center'},
                    {text: getIi18NText('createTime'), dataIndex: 'createTime', minWidth: 150, flex: 1, align: 'center'
...
//            }, 
    ...
    function statusFmtRender(value) {
    	if(value == 1){
    		return getIi18NText('normal');
    	} else {
    		return getIi18NText('paused');
    	}
    }
    // 砍价状态
    function activityFmtRender(value){
    	// 1:砍价中；2：已砍成；3：已使用；4：砍价失败；5：已过期
    	if(value == 1){
    		return '<font style="color:red;">'+getIi18NText('onGoing')+'</font>';
    	} else if (value == 2) {
    		return '<font style="color:green;">'+getIi18NText('cutCompleted')+'</font>';
    	} else if (value == 3) {
    		return '<font style="color:orange;">'+getIi18NText('used')+'</font>';
    	} else if (value == 4) {
    		return '<font style="color:gray;">'+getIi18NText('cutFail')+'</font>';
    	} else if (value == 5) {
    		return '<font style="color:gray;">'+getIi18NText('cutInvalid')+'</font>';
    	} else {
    		return '';
    	}
    }
    // 发起人
    function sponsorsFmtRender(value){
    	if(value == 1){
    		return '<font style="color:green;">'+getIi18NText('yes')+'</font>';
    	} else {
    ...
    function searchFn(btn) {
        ...
        cutActivityAjaxProxy.setExtraParam("startTime", params.startTime);
        cutActivityAjaxProxy.setExtraParam("endTime", params.endTime);
        cutActivityAjaxProxy.setExtraParam("search", params.search);

        cutActivityGridPanel.getSelectionModel().deselectAll();//此操作相当重要
        cutActivityDataStore.loadPage(1);
        Ext.getBody().unmask();
    }
...
//        ,listeners:{}
//    });
//    function opencutActivityWin(btn) {
//        if (cutActivityWin && cutActivityWin.isWindow) {
//            cutActivityWin.show(btn);
//            return
//        }
//        cutActivityWin = Ext.create('Ext.window.Window', {
//            id: "cutActivityWin",
//            frame: false,
//            modal: true,
//            constrain: true,
...
//    }
    ...
    //添加或修改赠券
//    function confirmcutActivity(btn) {
//        var cIds = [];
//        var uIds = [];
//        //校验优惠劵
//        var couponId1 = Ext.getCmp('couponId1').getValue();
...
//            })
//        }
//        cutActivityWin.getEl().mask(window.top.getIi18NText('saving'));
//        Ext.Ajax.request({
//            url: 'shared!savecutActivityRecord.action'
//            , params: {userIds: uIds.join(), couponIds: cIds.join()}
//            , method: 'post'
//            , callback: function (opt, success, response) {
//                cutActivityWin.getEl().unmask();
